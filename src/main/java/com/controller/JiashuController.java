...
public class JiashuController {
	...
	public R login(String username, String password, String captcha, HttpServletRequest request) {
		JiashuEntity user = jiashuService.selectOne(new EntityWrapper<JiashuEntity>().eq("jiashuzhanghao", username));
		if(user==null || !user.getMima().equals(password)) {
			return R.error("账号或密码不正确");
		}
		String token = tokenService.generateToken(user.getId(), username,"jiashu",  "家属" );
    ...
    public R register(@RequestBody JiashuEntity jiashu){
    	...
    	JiashuEntity user = jiashuService.selectOne(new EntityWrapper<JiashuEntity>().eq("jiashuzhanghao", jiashu.getJiashuzhanghao()));
		if(user!=null) {
    ...
    public R getCurrUser(HttpServletRequest request){
        ...
        JiashuEntity user = jiashuService.selectById(id);
	...
	public R remindCount(@PathVariable("columnName") String columnName, HttpServletRequest request, 
		...
		Wrapper<JiashuEntity> wrapper = new EntityWrapper<JiashuEntity>();
		if(map.get("remindstart")!=null) {
		...
		int count = jiashuService.selectCount(wrapper);
